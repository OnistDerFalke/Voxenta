...
bool ImGui::FileDialog(bool* open, ImFileDialogInfo* dialogInfo, ImVec2 mws)
{
	...
	static float initialSpacingColumn0 = 0.0f;
	static float initialSpacingColumn1 = 0.0f;
	static float initialSpacingColumn2 = 0.0f;
	...
	if (ImGui::Begin(dialogInfo->title.c_str(), open))
	{
		...
		ImGui::BeginChild("##browser", ImVec2(ImGui::GetWindowContentRegionWidth(), mws.y/2 - 100.0f), true, ImGuiWindowFlags_HorizontalScrollbar);
		ImGui::Columns(4);

		// Columns size
		if (initialSpacingColumn0 > 0)
		{
			ImGui::SetColumnOffset(0, initialSpacingColumn0);
			initialSpacingColumn0 = 0.0f;
		}
		if (initialSpacingColumn1 > 0)
		{
			ImGui::SetColumnOffset(1, initialSpacingColumn1);
			initialSpacingColumn1 = 0.0f;
		}
		if (initialSpacingColumn2 > 0)
		{
			ImGui::SetColumnOffset(2, initialSpacingColumn2);
