...
void node_editor::show()
{
    ...
    {
        cv::Mat viewer_image;
        ui_node* viewed_node = nullptr;
        if (viewer_node_id_ != -1 && find_ui_node(viewer_node_id_, &viewed_node))
            viewer_image = viewed_node->fx->get_preview_image();

        viewer_.show(viewer_image);
    }
